...
import { Star, ChevronLeft, ChevronRight, Heart, Minus, Plus } from "lucide-react";
...
export default function ProductDetailClient({ product, relatedProducts }: ProductDetailClientProps) {
  ...
  // Create 4 variations from the same product image
  const images = [
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <div className="max-w-[1440px] mx-auto w-full px-4 md:px-8 lg:px-[248px] py-3 md:py-4">
        <div className="flex items-center gap-1.5 md:gap-2 text-xs md:text-sm">
          <Link href="/" className="text-[#64748B] hover:text-primary">Home</Link>
        ...
        <div className="w-full">
            ...
            <div className="relative aspect-[4/3] flex items-center justify-center p-4 md:p-6 lg:p-8 overflow-hidden">
              ...
              <button
                ...
                className="absolute left-3 md:left-4 top-1/2 -translate-y-1/2 w-9 h-9 md:w-10 md:h-10 bg-white/90 rounded-full flex items-center justify-center shadow-lg hover:bg-white transition-colors"
              >
                <ChevronLeft className="w-4 h-4 md:w-5 md:h-5 text-primary" />
              </button>
              <button
                onClick={() => setSelectedImage((prev) => (prev < images.length - 1 ? prev + 1 : 0))}
                className="absolute right-3 md:right-4 top-1/2 -translate-y-1/2 w-9 h-9 md:w-10 md:h-10 bg-white/90 rounded-full flex items-center justify-center shadow-lg hover:bg-white transition-colors"
              >
            ...
            <div className="grid grid-cols-4 gap-2 md:gap-3 lg:gap-4 mt-3 md:mt-4">
              {images.map((img, index) => (
                <button
                  ...
                  className={`relative aspect-square rounded-lg md:rounded-xl overflow-hidden transition-all ${
                    selectedImage === index
          ...
          <div className="w-full">
            ...
            <div className="mb-5 lg:mb-6">
              ...
              <div className="flex flex-wrap gap-2">
                <span className="px-4 py-2 bg-[#F1F5F9] border border-[#E5E7EB] rounded-[10px] text-sm font-medium text-primary">
                  Apparel
                </span>
            ...
            <div className="flex items-center gap-4 mb-5 lg:mb-6">
              ...
              <div className="flex items-center border border-[#E5E7EB] rounded-[10px]">
                <button
                  ...
                  className="w-10 h-12 flex items-center justify-center text-[#64748B] hover:text-primary transition-colors"
                >
              ...
              {/* Wishlist Button */}
              <button className="flex items-center gap-2 px-6 h-12 border border-[#E5E7EB] rounded-[10px] hover:border-primary transition-colors">
                <Heart className="w-5 h-5 text-[#64748B]" />
                <span className="text-sm font-medium text-heading">Wishlist</span>
              </button>
            </div>

            {/* Add to Cart Button */}
            <button className="w-full h-12 bg-primary text-white font-medium text-sm lg:text-base rounded-[10px] shadow-[0px_4px_6px_rgba(0,0,0,0.1),0px_2px_4px_rgba(0,0,0,0.1)] hover:bg-primary-dark transition-colors mb-5 lg:mb-6">
              Add to Cart
            </button>
